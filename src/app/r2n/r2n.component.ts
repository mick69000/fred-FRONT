...
@Component({
  ...
  templateUrl: './r2n.component.html',
  styleUrl: './r2n.component.scss',
})
export class R2nComponent implements OnInit {
  ...
  récupèreLesDeuxMoisR2n(service: any, setter: (data: any) => void) {
    service.subscribe((data: any[]) => {
      const listeMoisEnCours = data.filter(
        (r2n) =>
          r2n.mois === this.moisEnCours && r2n.annee === this.anneeMoisEnCours
      );
      setter(listeMoisEnCours);
      const listeMoisPrecedent = data.filter(
        (r2n) =>
          r2n.mois === this.moisPrecedent &&
          r2n.annee === this.anneeMoisPrecedent
      );
  ...
  récupèreLesSetterDUnType(service: any, setter: (data: any) => void) {
    service.subscribe((data: any[]) => {
      const dataFiltres = data.filter((engin) => engin.type === 'R2N');
      setter(dataFiltres);
    });
  }
  ...
  valideLaJourneeR2n() {
    ...
    if (this.date !== '' && this.engin !== '' && this.agent1 !== '') {
      if (this.btnValider === 'Modifier') {
        this.dataR2nService.deleteR2n(this.date).subscribe(() => {});
      }
