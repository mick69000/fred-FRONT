...
export class AgcComponent implements OnInit {
  ...
  récupèreLesDeuxMoisAgc(service: any, setter: (data: any) => void) {
    service.subscribe((data: any[]) => {
      const listeMoisEnCours = data.filter(
        (agc) =>
          agc.mois === this.moisEnCours && agc.annee === this.anneeMoisEnCours
      );
      setter(listeMoisEnCours);
      const listeMoisPrecedent = data.filter(
        (agc) =>
          agc.mois === this.moisPrecedent &&
          agc.annee === this.anneeMoisPrecedent
      );
  ...
  récupèreLesSetterDUnType(service: any, setter: (data: any) => void) {
    service.subscribe((data: any[]) => {
      const dataFiltres = data.filter((engin) => engin.type === 'AGC');
      setter(dataFiltres);
    });
  }
